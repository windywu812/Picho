...
open class FatSecretClient {
     ...
     */
    public func searchFood(name: String, completion: @escaping (Result<Search, Error>) -> ()) {
        ...
        fatSecretRequest(with: components) { data in
            ...
            guard let model = self.retrieve(data: data, type: [String: Search].self) else {
                completion(.failure(ServiceError.noResult))
                return
            }
            guard let search = model["foods"] else {
                completion(.failure(ServiceError.noResult))
                return
            }

...
class MealsTodayView: UIView {
    ...
    @objc private func reloadDailyIntake(_: Notification) {
        fetchDailyIntake()
    ...
    @objc private func handleBreakfast(sender _: UITapGestureRecognizer) {
        let vc = FoodInputViewController()
        vc.eatingTime = .breakfast
        rootView.navigationController?.pushViewController(vc, animated: true)
    }

    @objc private func handleLunch(sender _: UITapGestureRecognizer) {
        let vc = FoodInputViewController()
        vc.eatingTime = .lunch
        rootView.navigationController?.pushViewController(vc, animated: true)
    }

    @objc private func handleDinner(sender _: UITapGestureRecognizer) {
        let vc = FoodInputViewController()
        vc.eatingTime = .dinner
        rootView.navigationController?.pushViewController(vc, animated: true)
    }

    @objc private func handleSnacks(sender _: UITapGestureRecognizer) {
        let vc = FoodInputViewController()
